...
@Component({
  selector: 'psm-scan',
  template: `
    <psm-widget key="hamlib" title="Hamlib Receiver Scanner">
      ...
      <div class="form-group freqlist">
        ...
        <div *ngFor="let freq of values.freqs; let n = index; let last = last">
          <ng-container *ngIf="freq.range == undefined">
            ...
            <div class="psm-input-group-4">
              <button psmInput class="form-control" [disabled]="! numeric(freq.freq) || ! freqsEnabled" (click)="onInsert(n)">Insert</button>
            </div>
            <div class="psm-input-group-4">
              <button psmInput class="form-control" [disabled]="values.freqs.length <= 2 || ! freqsEnabled" (click)="onDelete(n)">Delete</button>
            </div>
          </ng-container>
        </div>
