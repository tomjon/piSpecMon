...
@Component({
  ...
  template: `
    <psm-widget>
      <table class="table table-condensed">
        ...
        <tbody>
          <tr *ngFor="let config of configs" (click)="onSelect(config.id, $event)" [ngClass]="{'row-selected': selected == config.id, 'errors': config.errors}" [title]="config.values.description || '[no description]'">
  ...
  styles: [
    ...
    '.btn-selected { background: lightblue }',
    '.btn-right { float: right }',
    '.row-selected { background: lightblue }',
    '.errors { background: pink }',
    '.row-selected.errors { background: red }',
    '.running { color: green }',
